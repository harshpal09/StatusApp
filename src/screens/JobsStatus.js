import {
  ...
  export default function JobsStatus({ navigation ,route}) {
    ...
    navigation.setOptions({
        headerTitle:status == 'pending' ? 'Pending Jobs' :status == 'visits' ? 'Visits Done Jobs' : status == 'startworking' ? 'Work Done Jobs' : '' ,
    });
  }, []);
    ...
    const getData = async () => {
      try {
        ...
        if (response.data.data.code != undefined && response.data.data.code) {
          ...
          setFilter(response.data.data[jobstatus]);
        } else {
        }
      } catch (error) {
